...
my_directory <- here::here("site_testing",
                           site)


load(here::here("site_testing",
                site,
...
readr::write_csv(raw_data_j, here::here("site_testing",
                                        site,
                                        paste0(site, "_janelle.csv")))


calc_thresholds(SiteID = site, data = raw_data_j, directory = my_directory, units = "imperial")

source(here::here("RSS_MACA_Plot_Table_Creation.R"))

thresholds_j_f <- read_csv(here::here("site_testing",
                             site,
                             paste0(site, "_thresholds.csv")))
...
future_means_j <- readr::read_csv(here::here("site_testing",
                                             site,
                                             paste0(site, "_future_means.csv")))
...
mean_differences <- diffdf(future_means_c, future_means_j, file = here::here("site_testing",
                                                                             site,
                                                                             paste0(site,
                                                                                    "_mean_diff.txt")))
...
thresholds_j <- readr::read_csv(here::here("site_testing",
                                           site,
                                           paste0(site, "_thresholds.csv"))) %>%
...
threshold_differences <- diffdf(thresholds_c, thresholds_j,
                                file = here::here("site_testing",
                                                  site,
                                                  paste0(site, "_threshold_diff.txt")))
...
annual_cf_summary_j_1 <- readr::read_csv((here::here("site_testing",
                                                     site,
                                                     paste0(site, "_year_summary_q.csv")))) %>%
  filter(yr > 2025)
...
quadrant_differences <- diffdf(annual_cf_summary_c, annual_cf_summary_j,
                              file = here::here("site_testing",
                                                site,
                                                paste0(site,"_quadrant_diff.txt")))

# check model comparison


annual_cf_summary_j_c1 <- readr::read_csv(here::here("site_testing",
                                                     site,
                                                     paste0(site, "_year_summary_c.csv"))) %>%
  filter(yr > 2025)
...
corner_differences <- diffdf(annual_cf_summary_c_c, annual_cf_summary_j_c,
                             file = here::here("site_testing",
                                               site,
                                               paste0(site,site,"_corner_diff.txt")))
...
pca_summary <- readr::read_csv(here::here("site_testing",
                                          site,
                                          paste0(site, "_pca_summary.csv")))
...
pca_data <- readr::read_csv(here::here("site_testing",
                                       site,
                                       paste0(site, "_future_means_pca.csv")))
