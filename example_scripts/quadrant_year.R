...
quadrant_year <- cf_quadrant("BAND", data = thresholds, future_year = 2040, summarize_by = "year", method = "quadrant", directory = my_directory)
...
boxplot_graph <- function(variable){
  ...
  ggplot(data = quadrant_year, aes(x = time, y = .data[[variable]])) +
  ...
  ggsave(here::here(paste(SiteID, "_yearly_", variable, "_q.png",sep="")), width = 15, height = 9)
}
