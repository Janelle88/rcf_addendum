...
quadrant_season <- cf_quadrant("BAND", data = thresholds, future_year = 2040, summarize_by = "season", method = "quadrant", directory = my_directory)
...
bar_graph <- function(future1, future2, variable){
  ...
  quadrant_season_future <- quadrant_season %>%
    filter(cf %in% c(future1, future2)) %>%
    filter(time %in% c("Future"))

  ggplot(data = quadrant_season_future, aes(x = quarter, y = .data[[variable]])) +
  ...
  ggsave(here::here(paste(SiteID, "_season_", future1, "_", future2, "_", variable, "_q.png",sep="")), width = 15, height = 9)
